    ...
    public function getForDataTable($status = 1, $trashed = false)
    {
        ...
        if ($trashed == "true") {
            return User::onlyTrashed()
                ->select(['id', 'name', 'email', 'status', 'approved', 'confirmed', 'created_at', 'updated_at', 'deleted_at'])
                ->get();
        }

        return User::select(['id', 'name', 'email', 'status','approved', 'confirmed', 'created_at', 'updated_at', 'deleted_at'])
            ->where('status', $status)
            ->get();
    }
    ...
    public function update(User $user, $input, $roles)
    {
		...
		DB::transaction(function() use ($user, $input, $roles) {
			if ($user->update($input)) {
                ...
                $user->approved = isset($input['approved']) ? 1 : 0;
